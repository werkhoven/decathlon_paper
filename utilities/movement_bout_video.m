...
idx = cellfun(@(ii) ii.*3, idx, 'UniformOutput', false);
%%

cen = expmt.data.centroid.raw();
t = repmat(expmt.data.time.raw(),1,expmt.meta.num_traces);
speed = [NaN(1,expmt.meta.num_traces); ...
    squeeze(sqrt(sum(diff(cen,1,1).^2,2)))]./t;
...
for i=10:numel(idx)
    ...
    open(vid);
    nbouts = size(idx{i},1);
    roi = expmt.meta.roi.corners(i,:);
    roi_im = expmt.meta.ref.im;
    roi = [floor(roi(1:2)) ceil(roi(3:4))];
    imh.CData = roi_im(roi(2):roi(4),roi(1):roi(3));
