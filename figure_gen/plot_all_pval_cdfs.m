...
for j=1:4
    ...
    for i=1:numel(D_col)
        ...
        ph = plot_cdf_patch(p_tmp(pair_idx),'EdgeColor',colors(i),'FaceColor','none');
        uistack(ph,'bottom');
        xlabel('p-value');
        set(gca,'XLim',[0 1]);
    end
    ...
    title(sprintf('correlation matrix p-value CDFs (%s)',title_str));
    labels = arrayfun(@(i) sprintf('D%i',i), numel(D_col):-1:1, 'UniformOutput', false);
    legend([labels,{'uniform'}],'location','NorthWest');
end